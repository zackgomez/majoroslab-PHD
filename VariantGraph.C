...
void VariantGraph::phaseComponent(ConnectedComponent &component)
{
  ...
  for (int i = 0; i < N; ++i)
  {
    ...
    case UNPHASED:
      if (i != N - 1)
        throw "Unphased site in connected component";
      break;
    case IN_PHASE:
      break;
    case ANTI_PHASED:
      ...
      break;
    default:
      throw "No case in VariantGraph::phaseComponent";
    }
  }
}
...
void VariantGraph::assignReads()
{
       ...
       cur != end; ++cur)
  {
    ...
    if (read.shouldSkip())
      continue; // This read is marked to be skipped
    ReadVariants *mate = read.getMate();
    if (mate)
    {
      mate->skip(); // Mark the mate, so it gets skipped
    }
    ...
    if (compPhase == IN_PHASE)
      ...
      ++v.getCount(oppositeAllele(allele));
  }
}
...
bool VariantGraph::isSorted()
{
  ...
  for (int j = 0; j < numVariants - 1; ++j)
    if (variants[j].getPos() > variants[j + 1].getPos())
      return false;
  return true;
}
